...
contract IncrementalMerkleTree {
    ...
    mapping(uint32 => bytes32) public s_cacheSubtress;

    error IncrementalMerkleTree__DepthShouldBeGreaterThanZero();
    error IncrementalMerkleTree__DepthShouldBeLessThanMaxDepth();
    error IncrementalMerkleTree__IndexOutOfBounds(uint32 index);
    error IncrementalMerkleTree__MerkelTreeFull(uint32 nextLeafIndex);
    ...
    constructor(uint32 _depth, Poseidon2 _hasher) {
        ...
        if (_depth >= 32) {
            revert IncrementalMerkleTree__DepthShouldBeLessThanMaxDepth();
        }
    ...
    function _insert(bytes32 _leaf) internal returns (uint32 insertedIndex) {
        // add the leaf to the increamental merkle tree
        uint32 _nextLeafIndex = s_nextLeafIndex;
        if (_nextLeafIndex == uint32(2 ** i_depth)) {
            revert IncrementalMerkleTree__MerkelTreeFull(_nextLeafIndex);
        }
        ...
        for (uint32 i = 0; i < i_depth; i++) {
            if (currentIndex % 2 == 0) {
                ...
                left = currentHash;
                right = zeros(i);
                s_cacheSubtress[i] = currentHash;
            } else {
                // right
                //  - if odd, we need the leaf to be on the right and a cached subtree of the left
                // do this all the way up the tree
                left = s_cacheSubtress[i];
                right = currentHash;
            }
    ...
    function zeros(uint32 i) internal pure returns (bytes32) {
        // Returns the default zero value for a given tree depth
        // Each level is keccak256(previousLevel || previousLevel)
