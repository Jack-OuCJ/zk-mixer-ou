...
async function hashLeftRight(left, right) {
  ...
  // Convert decimal string to buffer to handle values that might be > field modulus
  const toBuffer32 = (decimalStr) => {
    const hex = BigInt(decimalStr).toString(16).padStart(64, '0');
    return Buffer.from(hex, 'hex');
  };
  const frLeft = Fr.fromBuffer(toBuffer32(left));
  const frRight = Fr.fromBuffer(toBuffer32(right));
